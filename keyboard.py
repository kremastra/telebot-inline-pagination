...
from telebot_pagination import start_button_pagination, edit_button_pagination
#from pandas import read_csv

#csv_airports = read_csv('D:/Work/Git/telebot-pagination/book_airports.csv', sep=';', header=None)
...
def demo_pagination(message):
    bot.send_message(message.from_user.id, text_message, reply_markup=start_button_pagination(list_of_tuples = data, rows_per_page=3))

@bot.callback_query_handler(func=lambda call: True)
def demo_pagination_handler(call: CallbackQuery):
    if call.data in ('previous_page', 'next_page'):
        bot.edit_message_text(text_message, reply_markup = edit_button_pagination(call, rows_per_page=3), chat_id = call.message.chat.id, message_id = call.message.message_id)
    for i in data:
        if call.data == i[1]:
            bot.send_message(
